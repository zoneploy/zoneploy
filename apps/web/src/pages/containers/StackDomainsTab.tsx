...
function StackZoneployRow({
  ...
  orgId: string
  stackId: string
  endpoint: StackZoneployEndpointInfo
  canManage: boolean
  onChanged: () => void
}) {
  ...
  const update = useMutation({
    mutationFn: () => stacksApi.updateZoneployDomain(orgId, stackId, endpoint.id, {
      slug: draftSlug.trim(),
  ...
  const remove = useMutation({
    mutationFn: () => stacksApi.removeZoneployDomain(orgId, stackId, endpoint.id),
    onSuccess: onChanged,
  })
...
export function StackDomainsTab({
  ...
  const addZoneploy = useMutation({
    mutationFn: () => stacksApi.addZoneployDomain(orgId, stackId, Number(newPort)),
    onSuccess: () => {
      setNewPort('')
      invalidate()
    },
