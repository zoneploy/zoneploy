...
export function StackInspectTab({ orgId, stackId, serviceName }: { orgId: string; stackId: string; serviceName: string }) {
  const { t } = useTranslation()
  const [copied, setCopied] = useState(false)

  const { data, isLoading, error } = useQuery({
    queryKey: ['stack-inspect', orgId, stackId, serviceName],
    queryFn: () => stacksApi.inspectService(orgId, stackId, serviceName),
    enabled: !!serviceName,
    staleTime: 10_000,
  })
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-40"><Loader size={20} className="animate-spin text-primary" /></div>
  }

  if (error) {
    return <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{t('containers.inspectError')}</div>
  }
  ...
  return (
    <div className="rounded-xl border border-grey-100 bg-background overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-grey-100">
        ...
        <button onClick={copyJson} className={cn('flex items-center gap-1.5 text-xs transition-colors', copied ? 'text-success' : 'text-text-secondary hover:text-text-primary')}>
